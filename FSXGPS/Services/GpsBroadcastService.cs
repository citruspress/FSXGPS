...
namespace FSXGPS.Services
{
    internal class GpsBroadcastService : IGpsBroadcastService
    {
        ...
        public void BroadcastGpsData(IFlightSimulatorDataService flightSimulatorDataService)
        {
        ...
        public void BroadcastAttitudeData(IFlightSimulatorDataService flightSimulatorDataService)
        {
            var heading = Math.Round(flightSimulatorDataService.Attitude.Heading, 3).ToString(CultureInfo.InvariantCulture);
            ...
            byte[] sendBytes = Encoding.ASCII.GetBytes(string.Format("XATTFSXGPS,{0},{1},{2}", heading, pitch, roll));
            _udpClient.Send(sendBytes, sendBytes.Length, _broadcastAddress);
        }
    }
}